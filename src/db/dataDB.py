...
async def db_select_hw_dail_price(query):
    ...
    if query.last_id == None:
        total_count = await prisma.query_raw(
            f"SELECT count(*) as rows FROM {table} use index(LDGMNT_DE) WHERE LDGMNT_DE = {query.ldgmnt_de}"
        )
        ...
        result = {'total_count':total_count[0]['rows'],'last_id':data[-1]['id'],'result':data}
    else:
        data = await prisma.query_raw(
