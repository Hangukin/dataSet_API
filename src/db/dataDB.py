...
async def db_select_hw_dail_price(query):
    ...
    if query.last_id == None: # 첫 페이지 호출 시 
        total_count = await prisma.query_raw(
            f"SELECT count(*) as total_rows FROM HW_LDGS_DAIL_MAX_AVRG_MIN_PRC_INFO WHERE LDGMNT_DE = {query.ldgmnt_de};"
        )
      
        data = await prisma.query_raw(
            f"SELECT * FROM HW_LDGS_DAIL_MAX_AVRG_MIN_PRC_INFO WHERE LDGMNT_DE = {query.ldgmnt_de} ORDER BY id ASC LIMIT 20000;"
            )
